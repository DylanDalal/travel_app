...
class _MyTripsSectionState extends State<MyTripsSection> {
  ...
  /// Load all city datasets from assets/trips/*.json
  Future<void> _loadCityDatasets() async {
    const List<String> datasetFiles = [
      'trips/africa.json',
      'trips/asia.json',
      'trips/europe.json',
      'trips/north_america.json',
      'trips/south_america.json',
      'trips/oceania.json',
      'trips/antarctica.json',
      'trips/central_america.json',
    ];

    Map<String, List<City>> loadedDatasets = {};
    List<City> combinedCities = [];

    for (String filePath in datasetFiles) {
      try {
        String data = await rootBundle.loadString(filePath);
        List<dynamic> jsonData = jsonDecode(data);
        List<City> cities = jsonData.map((e) {
          String name = e['name'] ?? 'Unknown';
          // Skip entries with name "Unknown"
          if (name == "Unknown") return null;
          double? lat = e['latitude']?.toDouble();
          double? lon = e['longitude']?.toDouble();
          if (lat == null || lon == null) return null;
          return City(name: name, latitude: lat, longitude: lon);
        }).whereType<City>().toList();

        loadedDatasets[filePath] = cities;
        combinedCities.addAll(cities);
      } catch (e) {
    ...
    print('Loaded ${allCities.length} cities from datasets.');
  }
  ...
  Future<void> fetchAndPlotPhotoMetadata() async {
    ...
    if (!state.isAuth) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Photo access denied.')),
      );
      return;
    }
    ...
    try {
      ...
      List<photo.AssetEntity> userPhotos =
          await recentAlbum.getAssetListPaged(page: 0, size: 100000);
  ...
  /// Find and print the closest city for each photo location
  void _findAndPrintClosestCities() {
    for (var location in photoLocations) {
      City? closestCity = findClosestCity(location.latitude, location.longitude);
      if (closestCity != null) {
        print(
            'Photo at (${location.latitude}, ${location.longitude}) is closest to ${closestCity.name}');
      } else {
        print(
            'Photo at (${location.latitude}, ${location.longitude}) has no nearby city found.');
      }
    }
  }

  /// Find the closest city using Geohash
  City? findClosestCity(double lat, double lon) {
    if (allCities.isEmpty) return null;

    // Calculate Geohash for the photo location
    final userGeohash = Geohash.encode(lat, lon, precision: 5); // Adjust precision as needed

    // Find cities within the same Geohash
    List<City> nearbyCities = allCities.where((city) {
      String cityGeohash = Geohash.encode(city.latitude, city.longitude, precision: 5);
      return cityGeohash == userGeohash;
    }).toList();

    // If no cities found in the same Geohash, consider all cities
    ...
    for (var city in nearbyCities) {
      double distance = calculateDistance(lat, lon, city.latitude, city.longitude);
  ...
  /// Plot photo locations on the Mapbox map
    ...
    if (isSelecting) {
      return Row(
        ...
        children: [
          ...
          TextButton(
            onPressed:
                (selectedTripIds.length > 1) ? _mergeSelectedTrips : null,
            child: Text(
              "Merge",
              style: TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.bold,
                color: (selectedTripIds.length > 1)
                    ? Colors.blue
                    : Colors.grey,
              ),
            ),
          ...
          TextButton(
            ...
            child: Text(
              ...
              style: TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.bold,
                color: selectedTripIds.isNotEmpty
                    ? Colors.red
                    : Colors.grey,
              ),
            ),
  ...
  Future<void> _performTripSplit(DateTime splitDate) async {
    ...
    final originalIndex =
        trips.indexWhere((t) => t['id'] == editingTripId);
    if (originalIndex < 0) {
    ...
    final String originalTitle =
        originalTrip['title'] ?? 'Untitled Trip';
    final Map<String, dynamic>? originalTimeframe =
        originalTrip['timeframe'];
    ...
    final String? startIso = originalTimeframe['start'];
    final String? endIso = originalTimeframe['end'];
    if (startIso == null || endIso == null) {
      print("Original trip timeframe is missing data. Cannot split.");
      return;
    }
    final DateTime startDate = DateTime.parse(startIso);
    final DateTime endDate = DateTime.parse(endIso);
    ...
    final List<dynamic> originalLocations =
        (originalTrip['locations'] ?? []) as List<dynamic>;
    ...
    for (var loc in originalLocations) {
      if (loc is Map<String, dynamic>) {
        final String? tsStr = loc['timestamp'] as String?;
        if (tsStr != null) {
          final DateTime locDate = DateTime.parse(tsStr);
    ...
    final Map<String, dynamic> newTrip = Map<String, dynamic>.from(originalTrip);
    ...
    final Map<String, dynamic> updatedOriginalTrip =
        Map<String, dynamic>.from(originalTrip);
    ...
    final List<Map<String, dynamic>> updatedTrips = List.from(trips);
                  ...
                  onPressed: () {
                    ...
                    setState(() {
                      ...
                      if (startIso.isNotEmpty && endIso.isNotEmpty) {
                        final DateTime start = DateTime.parse(startIso);
                        final DateTime end = DateTime.parse(endIso);
  ...
  Future<bool> _confirmSwipeDelete(Map<String, dynamic> trip) async {
    ...
    final String tripTitle = trip['title'] ?? 'Untitled Trip';
  ...
  Future<void> _deleteTrip(String tripId) async {
    ...
    try {
      ...
      final List<Map<String, dynamic>> updatedTrips =
  ...
  void _confirmDeleteSelected() {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        final int count = selectedTripIds.length;
        return AlertDialog(
          title: Text("Confirm Delete"),
          content: Text(
  ...
  void _mergeSelectedTrips() {
    ...
    final List<Map<String, dynamic>> selectedList =
        trips.where((t) => selectedTripIds.contains(t['id'])).toList();
    Map<String, dynamic>? earliestTrip;
    DateTime? earliestDate;

    for (var trip in selectedList) {
      final String? startStr = trip['timeframe']?['start'];
      if (startStr != null) {
        final DateTime startDate = DateTime.parse(startStr);
    ...
    final String defaultName =
        earliestTrip?['title'] ?? 'Merged Trip';
      ...
      builder: (BuildContext context) {
        return CupertinoAlertDialog(
          ...
          actions: [
            ...
            CupertinoDialogAction(
              child: Text("Merge & Keep Old Trips",
            ...
            CupertinoDialogAction(
              child: Text("Merge & Delete Old Trips",
                  textAlign: TextAlign.center),
              onPressed: () async {
                Navigator.of(context).pop();
                await _performTripMerge(
  ...
  Future<void> _performTripMerge(String mergedTripName,
    ...
    final List<Map<String, dynamic>> selected =
        trips.where((trip) => selectedTripIds.contains(trip['id'])).toList();
    if (selected.isEmpty) {
    ...
    for (var trip in selected) {
      final String? startStr = trip['timeframe']?['start'];
      final String? endStr = trip['timeframe']?['end'];
    ...
    final Map<String, String> mergedTimeframe = {
